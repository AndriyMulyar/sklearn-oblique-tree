...
int size, prev_size; {
  ...
  if (prev_size == 0) { //ANDRIY creating new array of POINTS
    if (array_name != NULL)
      free((char * )(array_name + 1));

    array_name = (struct point ** ) malloc((unsigned) size * sizeof(struct point * ));
    if (!array_name)
    ...
    array_name += 1;
    array_name = (struct point ** ) realloc(array_name, (unsigned) size * sizeof(struct point * ));
    if (!array_name)
